defmodule UnclickbaiterWeb.UserAuthTest do
  ...
  @remember_me_cookie "_unclickbaiter_web_user_remember_me"
  ...
  setup %{conn: conn} do
    conn =
      ...
      |> Map.replace!(
        :secret_key_base,
        UnclickbaiterWeb.Endpoint.config(:secret_key_base)
      )
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      UnclickbaiterWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    test "authenticates user from session", %{conn: conn, user: user} do
      ...
      assert conn.assigns.current_scope.user.authenticated_at ==
  ...
  describe "on_mount :mount_current_scope" do
    ...
    test "assigns nil to current_scope assign if there isn't a user_token", %{
      conn: conn
    } do
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{
      ...
      socket = %LiveView.Socket{
        endpoint: UnclickbaiterWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        UnclickbaiterWeb.Endpoint.subscribe(
          "users_sessions:#{Base.url_encode64(token)}"
        )
      end
