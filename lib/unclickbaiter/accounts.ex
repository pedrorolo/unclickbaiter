defmodule Unclickbaiter.Accounts do
  @moduledoc """
  ...
  alias Unclickbaiter.Accounts.{User, UserNotifier, UserToken}
  ...
  See `Unclickbaiter.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  def update_user_email(user, token) do
    ...
    Repo.transact(fn ->
      with {:ok, query} <-
           ...
           {:ok, user} <-
             Repo.update(User.email_changeset(user, %{email: email})),
           {_count, _result} <-
             Repo.delete_all(
  ...
  See `Unclickbaiter.Accounts.User.password_changeset/3` for a list of supported options.
